...
Lorin's copy from demo_search.py
...
def run_query_from_file(api: RiverscapesAPI, query_file: str, variables: dict = None):
    """Run a GraphQL query from a file and print the results."""
    ...
    # Provide default empty variables if none are passed
    variables = variables or {}

    # Run the query using the API
    response = api.run_query(query, variables)
    return response
...
if __name__ == '__main__':
    ...
    #
    # Also you need to call riverscapes_api.shutdown() when you're done with it.
    ...
    with RiverscapesAPI(stage='PRODUCTION') as riverscapes_api:
        ...
        # this works well 
        # query_file = os.path.join('graphql', 'queries', 'searchCount2.graphql')
        # variables = {}
        # result = run_query_from_file(riverscapes_api, query_file, variables)
