...
def add_projects_to_collection(riverscapes_api: RiverscapesAPI, env: str):
    ...
    if str.startswith(answers['method'], 'Cut'):
        ...
        with open(answers['file'], encoding='utf8') as f:
            guids = f.readlines()
    ...
    confirm = inquirer.prompt([inquirer.Confirm('continue', message=f"Continue adding {len(project_ids)} to collection: {answers['collection']}?")])
