...
from rsxml.project_xml import BoundingBox, Coords, Dataset, Geopackage, GeoPackageDatasetTypes, GeopackageLayer, Meta, MetaData, Project, ProjectBounds, Realization
...
def scrape_rme(rs_stage: str, rs_api: RiverscapesAPI, log_path: str, spatialite_path: str, search_params: RiverscapesSearchParams, download_dir: str, project_dir: str, project_name: str, delete_downloads: bool, min_rme_version) -> None:
    ...
    rs_project = Project(
        ...
        bounds=ProjectBounds(Coords(centroid[0], centroid[1]), BoundingBox(bounding_rect[0], bounding_rect[1], bounding_rect[2], bounding_rect[3]), os.path.basename(output_bounds_path)),
        realizations=[
            Realization(
                name='Realization1',
                xml_id='REALIZATION1',
                ...
                datasets=[
                    Geopackage(name='Riverscapes Metrics', xml_id='RME', path=os.path.relpath(output_gpkg, project_dir), layers=get_datasets(output_gpkg)),
                    Dataset(
                        xml_id='LOG',
                        ds_type='LogFile',
                        name='Lof File',
...
def get_datasets(output_gpkg: str) -> list[GeopackageLayer]:
    ...
    datasets = [GeopackageLayer(lyr_name=row[0], ds_type=GeoPackageDatasetTypes.VECTOR, name=row[0]) for row in curs.fetchall()]
    return datasets
...
def get_bounds(bounds_gpkg: str, spatialite_path: str) -> tuple[str, str, str]:
    ...
    bounding_box = [bounds_row[3], bounds_row[4], bounds_row[5], bounds_row[6]]

    geojson_output = {"type": "FeatureCollection", "features": [{"type": "Feature", "geometry": geojson_geom, "properties": {}}]}
